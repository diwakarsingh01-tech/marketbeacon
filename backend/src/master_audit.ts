...
function audit67(quotes, screener) {
...
function auditSR(quotes) {
  ...
  const cluster = (t) => {
    ...
    pivots.filter(p => p.t === t).forEach(p => {
      ...
      if (f) { z.m = (z.m * z.c + p.p) / (z.c + 1); z.c++; }
  ...
  for (const [name, list] of Object.entries(baskets)) {
    ...
    for (const sym of list) {
      ...
      results.forEach(r => {
        ...
        const auditPass = d.screener?.score >= 70 ? '🟢 YES' : '🔴 NO';
        if (isQualified || isRejected || list.length < 50) { // Limit output for large baskets
