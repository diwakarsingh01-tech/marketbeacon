...
export async function runHealthCheck(): Promise<{ results: HealthResult[]; passed: boolean; timestamp: string }> {
  ...
  await measure(results, 'SQLite/Turso Database', async () => {
    const db = getDB();
  ...
  await measure(results, 'Supabase', async () => {
    if (supabase) {
  ...
  await measure(results, 'Market Snapshot', async () => {
...

  {
    ...
    for (const label of ['Large Cap', 'Mid Cap', 'Small Cap']) {
      await measure(results, `Audit: ${label}`, async () => {
  ...
  await measure(results, 'Alpha-40 Cache', async () => {
  ...
  await measure(results, 'Key Symbol Coverage', async () => {
    const snap = getMarketSnapshot();
