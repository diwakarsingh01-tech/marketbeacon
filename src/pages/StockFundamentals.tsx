...
const StockFundamentalsPage: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col font-sans text-slate-200 bg-slate-950 lg:h-screen lg:overflow-hidden overflow-y-auto pb-24 md:pb-0 relative terminal-scan">
      
      {/* COMPACT HEADER */}
      <div className="bg-slate-950/95 backdrop-blur-md border-b border-slate-800 py-4 sticky top-0 z-10 shadow-xl">
        <div className="max-w-[1600px] mx-auto px-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex items-center space-x-6">
              <div className="space-y-0.5">
                 <div className="flex items-center space-x-2 text-[8px] font-bold text-slate-400 uppercase tracking-widest">
                    <Link to="/alpha-hub" className="hover:text-blue-400">Alpha Hub</Link>
                    <ChevronRight className="h-2 w-2" />
                    <span className="text-white">{symbol}</span>
                 </div>
                 <div className="flex items-center space-x-3">
                    <h1 className="text-2xl font-black text-white tracking-tighter uppercase leading-none">{symbol}</h1>
                    <span className="px-2 py-0.5 bg-slate-800 rounded text-[9px] font-bold text-slate-300 uppercase tracking-tighter">{data?.industry || 'General'}</span>
                    <div className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-tighter ${universe === 'INSTITUTIONAL' ? 'bg-blue-600 text-white' : 'bg-slate-700 text-white'}`}>{universe}</div>
                 </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between md:justify-end w-full md:w-auto space-x-8">
               <div className="flex flex-col items-end">
                 <span className="text-xl font-black text-white tracking-tighter leading-none">₹{data?.price?.toLocaleString() || '-'}</span>
               ...
               <div className="text-right">
                  ...
                  <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Audit Score</span>
               </div>
            </div>
        </div>
      </div>
      ...
      <main className="max-w-[1600px] mx-auto w-full flex-1 lg:overflow-hidden grid grid-cols-1 lg:grid-cols-12 gap-6 px-6 md:px-8 lg:px-10 py-8 relative">
        ...
        <div className={`lg:col-span-8 space-y-6 lg:overflow-y-auto pr-2 no-scrollbar transition-all duration-300 ${!isProOrAbove ? 'filter blur-[8px] pointer-events-none select-none opacity-40' : ''}`}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
             <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 flex flex-col justify-between h-24 transition-all duration-200 hover:border-blue-500/30">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Market Cap</span>
                <p className="text-lg font-black text-white leading-tight font-mono">{formatCr(data?.marketCap)}</p>
             </div>
             <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 flex flex-col justify-between h-24 transition-all duration-200 hover:border-blue-500/30">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Profitability (ROE)</span>
                <p className="text-lg font-black text-white leading-tight font-mono">{data?.returnOnEquity ? `${Number(data.returnOnEquity).toFixed(1)}%` : '-'}</p>
             </div>
             <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 flex flex-col justify-between h-24 transition-all duration-200 hover:border-blue-500/30">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Efficiency (ROCE)</span>
                <p className="text-lg font-black text-white leading-tight font-mono">{data?.roce ? `${Number(data.roce).toFixed(1)}%` : '-'}</p>
             </div>
             <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 flex flex-col justify-between h-24 transition-all duration-200 hover:border-blue-500/30">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Debt-To-Equity</span>
                <p className={`text-lg font-black leading-tight font-mono ${Number(data?.netDebtToEquity) > 0.2 ? 'text-red-500' : 'text-white'}`}>{Number(data?.netDebtToEquity).toFixed(2)}</p>
             </div>
          </div>

          <section className="bg-slate-900/60 border border-slate-800 rounded-2xl shadow-xl overflow-hidden backdrop-blur-sm">
             <div className="px-6 py-4 border-b border-slate-800 bg-slate-950/50 flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-slate-300">
                <span className="text-white">Institutional Audit Matrix</span>
                ...
                {weightedSegments.map((segment) => segment.data && (
                  <div key={segment.id} className="space-y-3">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                      <h3 className="text-[9px] font-bold text-slate-400 uppercase tracking-widest flex items-center">
                        {segment.icon} {segment.label}
                      </h3>
                      <span className="text-[9px] font-bold text-white">{segment.data.score}/{segment.data.max}</span>
                    </div>
          ...
          <section className="bg-slate-900/60 border border-slate-800 rounded-2xl shadow-xl p-6 grid grid-cols-2 md:grid-cols-4 gap-6 backdrop-blur-sm">
             {[
               ...
               <div key={i} className="space-y-1">
                  ...
                  <p className="text-xs font-bold text-white uppercase leading-none font-mono">{item.value}</p>
               </div>
             ))}
          </section>
        </div>
        ...
        <div className={`lg:col-span-4 space-y-6 h-full transition-all duration-300 ${!isProOrAbove ? 'filter blur-[8px] pointer-events-none select-none opacity-40' : ''}`}>
           <div className="bg-slate-900/60 border border-slate-800 rounded-2xl shadow-xl p-6 space-y-6 backdrop-blur-sm">
              <h3 className="text-[9px] font-bold text-slate-400 uppercase tracking-widest flex items-center">
                <Target className="h-4 w-4 mr-2 text-blue-500" /> Valuation & Ownership
              </h3>
              <div className="space-y-4">
                 <div className="grid grid-cols-2 gap-4">
                    <div className={`p-4 rounded-xl border ${isPEOvervalued ? 'bg-red-500/10 border-red-500/30' : 'bg-slate-950 border-slate-800'} transition-all duration-200 hover:scale-[1.02]`}>
                       <p className={`text-[7px] font-bold uppercase ${isPEOvervalued ? 'text-red-500' : 'text-slate-400'}`}>Current PE</p>
                       <p className={`text-lg font-black leading-none font-mono ${isPEOvervalued ? 'text-red-500' : 'text-white'}`}>{peRatio.toFixed(1)}</p>
                    </div>
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 text-center transition-all duration-200 hover:scale-[1.02]">
                        <p className="text-[7px] font-bold text-slate-400 uppercase">Median P/E (3Y / 5Y)</p>
                 ...
                 <div className="space-y-3">
                    {[
                      ...
                      <div key={idx} className="space-y-2">
                         <div className="flex justify-between items-center text-[9px] font-bold">
                            <span className="text-slate-400 uppercase">{holder.label}</span>
                            <span className="text-white">{holder.value?.toFixed(1)}%</span>
                         </div>
                    ...
                    <div className="pt-2 flex flex-col">
                        <span className="text-[9px] font-bold text-slate-400 uppercase">Smart Money Total</span>
                        <span className="text-2xl font-black text-white tracking-tighter leading-none font-mono">{data?.shareholding?.smartMoneyTotal?.toFixed(2)}%</span>
                    </div>
                 </div>
              </div>
           </div>

           <div className="bg-slate-950 rounded-2xl p-6 text-white space-y-4 shadow-xl border border-slate-800 backdrop-blur-sm">
              <h3 className="text-xs font-black uppercase tracking-widest italic">Research Hub</h3>
              <div className="grid grid-cols-1 gap-3">
                 <a href={`https://www.tradingview.com/symbols/NSE-${symbol}`} target="_blank" className="flex items-center justify-between p-4 bg-white/5 rounded-xl hover:bg-white/10 transition-all border border-white/10 group" rel="noreferrer">
                 ...
                 <a href={`https://www.screener.in/company/${symbol}/consolidated/`} target="_blank" className="flex items-center justify-between p-4 bg-white/5 rounded-xl hover:bg-white/10 transition-all border border-white/10 group" rel="noreferrer">
        ...
        {/* Lock Overlay */}
        {!isProOrAbove && (
          <div className="absolute inset-0 z-20 flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-8 rounded-2xl">
            <div className="bg-slate-950 text-white rounded-3xl p-10 max-w-md w-full shadow-2xl border border-slate-800 text-center space-y-8 animate-in fade-in zoom-in-95 duration-200">
               ...
               <div className="space-y-4 pt-2">
                  <button 
                    ...
                    className="w-full py-5 bg-blue-600 text-white rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-blue-500 hover:scale-[1.02] active:scale-95 transition-all shadow-lg shadow-blue-900/30 flex items-center justify-center gap-2"
                  >
               ...
               <div className="border-t border-slate-800 pt-8 space-y-5">
                  ...
                  <div className="flex gap-3">
                     <input 
                       ...
                       className="flex-1 bg-slate-900 border border-slate-700 px-5 py-4 rounded-xl text-xs font-black uppercase tracking-widest text-white outline-none focus:border-blue-500 placeholder:text-slate-600"
                     />
                     <button
                       onClick={handleRedeemVoucher}
                       disabled={redeeming}
                       className="px-6 py-4 bg-white text-slate-950 hover:bg-slate-100 disabled:bg-slate-800 disabled:text-slate-600 rounded-xl text-xs font-black uppercase tracking-widest transition-all"
                     >
