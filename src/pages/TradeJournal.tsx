...
const TradeJournal: React.FC = () => {
  ...
  const fetchTrades = useCallback(async () => {
    ...
    if (!token) return;
    try {
      const res = await fetch('http://127.0.0.1:3001/api/trades', {
  ...
  const fetchLivePrices = async (symbols: string[]) => {
    try {
      const res = await fetch(`http://127.0.0.1:3001/api/stocks?symbols=${symbols.join(',')}`);
      if (res.ok) {
        const data = await res.json();
        const prices: any = {};
  ...
  const handleBulkDelete = async () => {
    ...
    const token = localStorage.getItem('mb_token');
    try {
      const res = await fetch('http://127.0.0.1:3001/api/trades/batch-delete', {
        method: 'POST',
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        ...
        try {
          ...
          const res = await fetch('http://127.0.0.1:3001/api/trades/batch', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleConfirmClose = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:3001/api/trades/${showCloseModal.id}/close`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ 
  ...
  const handleReopenTrade = async (id: number) => {
    ...
    const token = localStorage.getItem('mb_token');
    try {
      const res = await fetch(`http://127.0.0.1:3001/api/trades/${id}/reopen`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDeleteTrade = async (id: number) => {
    ...
    const token = localStorage.getItem('mb_token');
    try {
      const res = await fetch('http://127.0.0.1:3001/api/trades/' + id, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) fetchTrades();
