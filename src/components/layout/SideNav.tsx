    ...
    `}>
      ...
      <div className="p-6 pb-8 border-b border-white/[0.03] flex items-center justify-between">
        ...
        <div className="md:hidden">
           <button onClick={onClose} className="p-2 text-slate-500 hover:text-white transition-all rounded-xl bg-white/5 border border-white/10 active:scale-95">
      ...
      <nav className="flex-1 px-3 py-6 space-y-8 overflow-y-auto no-scrollbar">
            ...
            <div className="space-y-1">
              {section.items.map((item) => (
                <NavLink
                  ...
                  onClick={onClose}
                  className={({ isActive }) => `
                    flex items-center group px-5 py-3 transition-all duration-300 relative
                    ${isActive 
                      ? 'bg-gradient-to-r from-blue-600/15 to-transparent text-white border-l-[4px] border-blue-500' 
                      : 'text-slate-400 hover:text-white hover:bg-white/[0.02] border-l-[4px] border-transparent hover:translate-x-1'}
                  `}
                >
                  {({ isActive }) => (
                    <>
        ...
        <div className="px-3 pt-4">
          <a 
            ...
            className="flex flex-col p-4 bg-gradient-to-br from-blue-600/10 to-indigo-600/5 hover:from-blue-600/15 hover:to-indigo-600/10 border border-blue-500/20 hover:border-blue-500/40 rounded-2xl group transition-all shadow-md relative overflow-hidden"
          >
        ...
        {isAdmin && (
          <div className="pt-4 border-t border-white/[0.03]">
            <NavLink
              ...
              onClick={onClose}
              className={({ isActive }) => `
                flex items-center px-5 py-4 transition-all duration-300 relative group
                ${isActive 
                  ? 'bg-gradient-to-r from-blue-600/15 to-transparent text-white border-l-[4px] border-blue-500' 
      ...
      <div className="p-6 mt-auto border-t border-white/[0.03] space-y-4 bg-slate-950/20">
        <div className="flex items-center justify-between">
           <div className="flex items-center gap-1.5">
              <Link to="/profile" onClick={onClose} className="p-2 text-slate-500 hover:text-white transition-all rounded-lg hover:bg-white/5 border border-transparent hover:border-white/5" title="Settings">
                 <Settings className="h-4 w-4" />
              </Link>
              <Link to="/education" onClick={onClose} className="p-2 text-slate-500 hover:text-white transition-all rounded-lg hover:bg-white/5 border border-transparent hover:border-white/5" title="Help">
